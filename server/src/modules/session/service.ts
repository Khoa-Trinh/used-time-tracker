...
export abstract class SessionService {
    static async logSession(params: LogSessionParams) {
        ...
        if (durationMs > 24 * 60 * 60 * 1000) throw new Error('Duration cannot exceed 24 hours');
        try {
    ...
    private static async upsertCoreData(tx: any, params: LogSessionParams, start: Date) {
        ...
        if (!app) {
            // Use AI Service for better suggestion (attempts OpenAI -> Local fallback)
            const suggestion = await AIService.suggestCategory({ appName, url, userId });
            let finalCategory = suggestion.category;
            let autoSuggested = true;

            // Check URL patterns for override (User preferences trump AI)
            if (url) {
                const userPatterns = await tx.select().from(urlPatterns).where(
                    eq(urlPatterns.userId, userId)
                ).orderBy(desc(urlPatterns.priority));

                for (const pattern of userPatterns) {
                    if (matchUrlPattern(url, pattern.pattern)) {
                        finalCategory = pattern.category;
                        autoSuggested = false; // User defined pattern
                        break;
                    }
                }
            }
